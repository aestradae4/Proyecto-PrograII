package com.candysrouse.API;
//Mapeo URL DE API

import com.candysrouse.DAO.candyApiFacturaDAO;
import com.candysrouse.Model.facturaModel;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;
import javax.ws.rs.Consumes;
import javax.ws.rs.DELETE;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.PUT;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;

@Path("/factura")
//Se va a consumir consultas JSON
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class candyApiFacturaAPI {
    //Inicializar el objeto
    candyApiFacturaDAO facturaDAO = new candyApiFacturaDAO();

    //Variable de resultados
    boolean res;

    //Ingresar usuario 
    /*
    Post: Sirve para enviar la informacion 
     */
    @POST
    public Response agregarFactura(facturaModel factura) throws SQLException {
        res = facturaDAO.agregar(factura);
        if (res) {
            return Response.status(Response.Status.CREATED).build();
        } else {
            return Response.status(500, "Ocurrio un error al registrar el usuario").build();
        }
    }

    /*
    Metodo Get: 
    Se utiliza para obtener la lista de objetos, segun la tabla en la base de datos
     */
    @GET
    public List<facturaModel> obtenerFactura() {
        List<facturaModel> listaFactura = new ArrayList<facturaModel>();
        listaFactura = facturaDAO.obtenerTodos();
        return listaFactura;
    }

    /*
    @GET by ID: 
    Se utiliza para obtener un objeto especifico segun una tabla de la base de datos
     */
    @GET
    @Path("/{idFactura}")
    public facturaModel obtenerUnaFactura(@PathParam("idFactura") int idFactura) {
        facturaModel factura = new facturaModel();
        factura = facturaDAO.obtenerUnaFactura(idFactura);
        if (factura != null) {
            return factura;
        } else {
            return factura;
        }

    }
    
    /*
    Metodo DELETE 
    Encargado de eliminar un registro en especifico de la base de datos
    */
    @DELETE
    @Path("/{idFactura}")
    public Response eliminarFactura(@PathParam("idFactura")int idFactura){ 
        res = facturaDAO.eliminar(idFactura); 
        if(res){ 
            return Response.status(200, "Registro eliminado correctamente").build();
        }else { 
            return Response.status(500, "Ocurrio un error al eliminar el registro").build(); 
        }
    }
    /*
    Metodo PUT:
    Encargado de actualizar datos en un registro en especifico en la DB
    */
   @PUT
   //@Path("/{idFactura}")
   public Response actualizarFactura(facturaModel faacturaActualizado) throws SQLException{
       res = facturaDAO.actualizar(faacturaActualizado);
       if(res){ 
           return Response.ok(faacturaActualizado).build(); 
       }else {
           return Response.status(500, "Error al actualizar el usuario").build();
       }
   }
    
}
