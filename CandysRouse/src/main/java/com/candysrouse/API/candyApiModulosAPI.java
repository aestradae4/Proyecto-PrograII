package com.candysrouse.API;
//Mapeo URL DE API

import com.candysrouse.DAO.candyApiModulosDAO;
import com.candysrouse.Model.modulosModel;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;
import javax.ws.rs.Consumes;
import javax.ws.rs.DELETE;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.PUT;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;

@Path("/modulos")
//Se va a consumir consultas JSON
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class candyApiModulosAPI {
    
    //Inicializar el objeto
    candyApiModulosDAO modulosDAO = new candyApiModulosDAO();

    //Variable de resultados
    boolean res;

    //Ingresar usuario 
    /*
    Post: Sirve para enviar la informacion 
     */
    @POST
    public Response agregarModulos(modulosModel modulos) throws SQLException {
        res = modulosDAO.agregar(modulos);
        if (res) {
            return Response.status(Response.Status.CREATED).build();
        } else {
            return Response.status(500, "Ocurrio un error al registrar el usuario").build();
        }
    }

    /*
    Metodo Get: 
    Se utiliza para obtener la lista de objetos, segun la tabla en la base de datos
     */
    @GET
    public List<modulosModel> obtenerModulos() {
        List<modulosModel> listaModulos = new ArrayList<modulosModel>();
        listaModulos = modulosDAO.obtenerTodos();
        return listaModulos;
    }

    /*
    @GET by ID: 
    Se utiliza para obtener un objeto especifico segun una tabla de la base de datos
     */
    @GET
    @Path("/{idModulo}")
    public modulosModel obtenerModulos(@PathParam("idModulo") int idModulo) {
        modulosModel modulos = new modulosModel();
        modulos = modulosDAO.obtenerUnModulo(idModulo);
        if (modulos != null) {
            return modulos;
        } else {
            return modulos;
        }

    }

    /*
    Metodo DELETE 
    Encargado de eliminar un registro en especifico de la base de datos
     */
    @DELETE
    @Path("/{idModulo}")
    public Response eliminarModulos(@PathParam("idModulo") int idModulo) {
        res = modulosDAO.eliminar(idModulo);
        if (res) {
            return Response.status(200, "Registro eliminado correctamente").build();
        } else {
            return Response.status(500, "Ocurrio un error al eliminar el registro").build();
        }
    }

    /*
    Metodo PUT:
    Encargado de actualizar datos en un registro en especifico en la DB
     */
    @PUT
   
    public Response actualizarModulos(modulosModel modulosActualizado) throws SQLException {
        res = modulosDAO.actualizar(modulosActualizado);
        if (res) {
            return Response.ok(modulosActualizado).build();
        } else {
            return Response.status(500, "Error al actualizar el usuario").build();
        }
    }
}
